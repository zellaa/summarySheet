...
\textbf{Scheme:} For e.g. (on finite domain) $u_t = c u_{xx}$ with $x_j, t_m$, we have scheme for $1 \leq j \leq J-1, 0\leq m \leq M-1$, and initial conditions $\forall j$.
...
\textbf{L$_2$ F'n:} We approximate $f_{i,j} \rightarrow \frac{1}{h^2}  \int_{K_{i,j}} f$ where $K_{i,j} = [x_i \pm 1/2, y_i \pm 1/2]$. \textcolor{blue}{For errors:} NB that $Au-AU =  -D_x^+D_x^- u -D_y^+ D_y^- u + cu - T(\Delta u + cu)$.\textcolor{blue}{CONTINUED}\\ NB $Tu_{xx} = D_x^+ \frac{1}{h} \int u_x (x_i - \frac{h}{2} )dy := D_x^+ \alpha_x$ so $Ae_{i,j} = D_x^+ \phi_1 + D_x^- \phi_2 + \psi$, with $\phi_1:= \alpha_x - D_x^- u, \psi := cu - Tcu$. Now NB $c_0 \norm{e}_{1,h}^2 \leq (Ae,e)$. Bound $(D_x^+ \phi_1,e)$ via $\leq || \phi_1 ]|_x ||D_x^- e]|_h$ so $c_0 \norm{e}_{1,h}^2 = (|| \phi_1 ]|^2_x +|| \phi_2 ]|^2_y+ \norm{\psi}_h^2) \norm{e}_{1,h}$
\end{document}
