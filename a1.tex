...
\begin{document} \noindent
\textcolor{red}{APDE:} \textbf{Charpit:} $F(p,q,u,x,y)=0$ with $\color{blue}{u_x = p,u_y=q,\dot x= F_p,\dot y=F_q}$. Then via $F_x,F_y, \& \: p_y=q_x \rightarrow \color{blue}{p_\tau = -F_x - pF_u}$, $\color{blue}{q_\tau = -F_y-qF_u}, \color{blue}{u_\tau = pF_p + qF_q}$. Also, $\color{blue}{ \frac{du_0}{ds}  = p_0 \frac{dx_0}{ds}  + q_0 \frac{dy_0}{ds} ; F_0 = 0}$ - last 2 needed to show $u$ defined on $\Gamma$.
\textbf{Proof of Charpit:} Write $ \frac{dF}{d\tau}  = F_p \frac{dp}{d\tau} + F_q \frac{dq}{d\tau } + F_u \frac{du}{d\tau} + F_x \frac{dx}{d\tau } + F_y\frac{dy}{d\tau} =0 $. Then write $\phi := u_s -p x_s -q y_s$. Take $\phi_\tau$ to get $\phi_\tau = F_s - \phi F_u + \partial_s \left( u_\tau -p F_p -q F_q \right) \rightarrow \phi_\tau = - \phi F_u$. Then we have $u_\tau = x_\tau u_x + y_\tau y_x$, and $u_s = x_s p + y_s q$ which means $p:= u_x, q:=u_y$.
